...
const { connectToImap, fetchEmails, parseEmail } = require('./utils/emailUtils');
...
async function addEventToCalendar(auth, eventDetails) {
  ...
  const event = {
    ...
    start: { dateTime: eventDetails.startDateTime.toISOString(), timeZone: 'Asia/Kolkata' },
    end: { dateTime: eventDetails.endDateTime.toISOString(), timeZone: 'Asia/Kolkata' },
    reminders: {
      useDefault: false,
      overrides: [
...
async function getUserCriteria() {
  ...
  const since = await askQuestion("Fetch emails since (YYYY-MM-DD, leave blank for last 24 hours): ");
  const maxEmails = await askQuestion("Maximum number of emails to fetch (leave blank for all): ");
...
Date: ${email.date}
Body (preview): ${email.text.substring(0, 100)}${email.text.length > 100 ? '...' : ''}
Attachments: ${email.attachments.length > 0 ? email.attachments.map(a => `- ${a.filename}`).join(', ') : 'None'}
Links: ${email.links.length > 0 ? email.links.map(l => `- ${l.text}: ${l.href}`).join('\n       ') : 'None'}
`);
  });
